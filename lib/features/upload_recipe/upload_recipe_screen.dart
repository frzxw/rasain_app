...
class _UploadRecipeScreenState extends State<UploadRecipeScreen>
  ...
  void initState() {
    ...
    // Listen untuk auth state changes dengan tambahan stream listener
  ...
  void _refreshAuthState() {
    ...
    // Secara periodik check auth state sampai user login
    int attempts = 0;
    while (attempts < 30 && mounted) { // max 15 detik
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocConsumer<UploadRecipeCubit, UploadRecipeState>(        listener: (context, state) {
  ...
  Widget _buildUploadButton(UploadRecipeState state) {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: isFormValid && state.status != UploadRecipeStatus.loading
            ? _uploadRecipe
            : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primary,
        ...
        child: state.status == UploadRecipeStatus.loading
            ? const Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  SizedBox(
